...
%     s = 0;
    ...
    for gainset = damel
%         load("optimised_DDLV/gain_strain/unit_perturbations/gains_" + num2str(gainset))  
        load("optimised_DDLV/gain_strain/s_vals/gains_" + num2str(gainset)) 
        K = gains{1,1};
        norm_K = gains{1,2};

        for s_val = 1:numel(Lambda)
            ...
            for run = 0:999
                ...
                strains(:, tot_runs+1, 1) = eps_OL;
                strains(:, tot_runs+1, 2) = eps_CL;
...
strains_mean = [mean(strains(:, :, 1), 2), mean(strains(:, :, 2), 2)];
strains_mean = strains_mean ./ max(strains_mean)

sucecss_rates = round(success_rates,2)
...
bar_data = [strains_mean(:,1); strains_mean(:,2)];
...
percentile = 10;
for i = 1:2*n_dof
    end_pos = [prctile(strains2(i,:), percentile), prctile(strains2(i,:), 100-percentile)];
%     an = annotation('arrow');
%     an.Parent = f2.CurrentAxes;
%     an.X = [i, i];
%     an.Y = end_pos;
%     an.Color = [1, 0, 0];
...
l = legend('Mean strain (OL)', 'Mean strain (CL)', percentile_str, 'location', 'south west');
