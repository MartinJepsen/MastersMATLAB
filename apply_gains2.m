...
for i_e = elements
    ...
    n_poles = numel(poles);
    n_runs = n_sim * n_sim_d * n_poles;



    strains = zeros(size(B_strain, 1), n_runs, 2);
    min_strain_OL = zeros(n_poles, 1);
    min_strain_CL = zeros(n_poles, 1);
...
%                 min_strain_CL(tot_runs, 1) = find(eps_norm_CL == min(eps_norm_CL));   % index of smallest CL strain
        ...
        eps_norm = sum(eps_p ./ max(eps_p),2);
        
        min_strain_OL(polenum, 1) = find(eps_norm(:,:,1) == min(eps_norm(:,:,1),[],'all'));   % index of smallest OL strain
        min_strain_CL(polenum, 1) = find(eps_norm(:,:,2) == min(eps_norm(:,:,2),[],'all'));   % index of smallest CL strain
